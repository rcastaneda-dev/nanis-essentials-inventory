-- Drop competitor price columns from products (no longer used on frontend)
-- Also update products_with_image_info view that depended on these columns

DROP VIEW IF EXISTS public.products_with_image_info;

ALTER TABLE public.products
  DROP COLUMN IF EXISTS competitor_a_price,
  DROP COLUMN IF EXISTS competitor_b_price;
...
GROUP BY
    p.id, p.name, p.sku, p.description, p.notes, p.category, p.weight_lbs,
